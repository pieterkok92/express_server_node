...
router.addRoute('/images', function (req, res, params) {
  if (req.method === 'POST') {
    ...
    busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
      ...
      // Handle uploading file to Amazon S3.
      // We are passing 'file' which is a ReadableStream, filename' which is the name of the file and a callback function to handle success/error.
      uploadToS3(file, filename, function (err) {
        if (err) {
          res.statusCode = 500;
          res.end(err);
        } else {
